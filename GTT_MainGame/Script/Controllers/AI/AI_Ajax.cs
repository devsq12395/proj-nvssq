...
public class AI_Ajax : MonoBehaviour {
	public static AI_Ajax I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 50){
			if (enemyCount <= 2) {
				#region "All-out assault"
				MG_ControlCommand.I.gameAction_ajax_allOutAssault(current);
				#endregion
			}else{
				#region "Hold the line"
				MG_ControlCommand.I.gameAction_ajax_holdTheLine(current);
				#endregion
			}
		}
