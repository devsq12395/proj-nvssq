...
public class AI_ET : MonoBehaviour {
	public static AI_ET I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 75){
			if (enemyCount <= 2 && allyCount >= 2) {
				#region "Entangle"
				MG_ClassUnit target = getSingleAttackTarget (current, 4);
				MG_ControlCommand.I.gameAction_elderTreant_entangle (current, target.posX, target.posY);
				#endregion
			}else{
				#region "Root Armor"
				MG_ClassUnit target = getHealTarget (current, 4);
				MG_ControlCommand.I.gameAction_elderTreant_rootArmor (current, target.posX, target.posY);
				#endregion
			}
		}
