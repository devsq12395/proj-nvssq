...
public class AI_Eve : MonoBehaviour {
	public static AI_Eve I;
		...
		if(hasTarget && current.MP >= 50){
			if (enemyCount >= 2) {
				if (current.HP >= 150) {
					...
					MG_ControlCommand.I.gameAction_eve_deathArrow(current, target.posX, target.posY);
					#endregion
				} else {
					#region "Drain Life"
					MG_ClassUnit target = getSingleAttackTarget(current, 4);
					MG_ControlCommand.I.gameAction_eve_drainLife(current, target.posX, target.posY);
					#endregion
				}
			}
		}
