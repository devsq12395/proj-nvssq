...
public class AI__BOSS_Aleric : MonoBehaviour {
	public static AI__BOSS_Aleric I;
	...
	public void _runAI(MG_ClassUnit current, Vector2 target, int moveToDo){
		...
		if (turnState < 3) {
			...
			switch(moveToDo){
				...
				case 2: 				useAbility (current, 1); break;
				case 3: 				useAbility (current, 2); break;
				case 4: 				useAbility (current, 1); break;
	...
	public void useAbility(MG_ClassUnit current, int abilityToUse){
		...
		if(hasTarget){
			if (abilityToUse == 1) {
				...
				MG_ControlCommand.I.gameAction_aleric_axeThrow(current, target.posX, target.posY, "AxeThrow(Boss)", 100);
				#endregion
			} else {
				#region "Mighty Slam"
				MG_ClassUnit target = getSingleAttackTarget(current, 4);
				MG_ControlCommand.I.gameAction_aleric_mightySlam(current, target.posX, target.posY, true, 100);
				#endregion
			}
		}
