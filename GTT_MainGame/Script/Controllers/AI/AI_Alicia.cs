...
public class AI_Alicia : MonoBehaviour {
	public static AI_Alicia I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		assesSituation (current, 1);

		//Uses the ability on a target enemy
		if(current.MP >= 50){
			if (hasTarget) {
				#region "Close Combat"
					MG_ClassUnit target = getSingleAttackTarget (current, 1);
					MG_ControlCommand.I.gameAction_alicia_closeCombat(current, target.posX, target.posY);
					#endregion
			} else {
				#region "Summon Recruit"
				if(MG_Globals.I.players[current.playerOwner]._HERO_getSummonedUnitCount(current.name) < 1){
					MG_ControlCommand.I.gameAction_alicia_summonRecruit(current, current.posX, current.posY);
				}
				#endregion
			}
		}
