...
public class AI_Victoria : MonoBehaviour {
	public static AI_Victoria I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 50){
			if (allyCount <= 2) {
				...
				MG_ControlCommand.I.gameAction_victoria_heal (current, target.posX, target.posY);
				#endregion
			}else{
				#region "Shell"
				MG_ClassUnit target = getHealTarget (current, 4);
				MG_ControlCommand.I.gameAction_victoria_shell (current, target.posX, target.posY);
				#endregion
			}
		}
