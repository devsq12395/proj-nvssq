...
public class AI_Elise : MonoBehaviour {
	public static AI_Elise I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		assesSituation (current, 2);

		//Uses the ability on a target enemy
		if(hasTarget){
			MG_ClassUnit target = getSingleAttackTarget (current, 2);
			MG_ControlCommand.I.gameAction_elise_multiStrike(current, target.posX, target.posY);
		}
