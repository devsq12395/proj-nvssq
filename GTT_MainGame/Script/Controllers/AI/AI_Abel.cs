...
public class AI_Abel : MonoBehaviour {
	public static AI_Abel I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 50){
			...
			if (enemyCount >= 2) {
				#region "Test of Faith - Ally"
				MG_ClassUnit target = getHealTarget(current, 4);
				Debug.Log("ally targeting " + target.posX + ", " + target.posY);
				MG_ControlCommand.I.gameAction_abel_testOfFaith(current, target.posX, target.posY);
				#endregion
			} else {
				#region "Test of Faith - Enemy"
				MG_ClassUnit target = getSingleAttackTarget(current, 4);
				Debug.Log("enemy targeting " + target.posX + ", " + target.posY);
				MG_ControlCommand.I.gameAction_abel_testOfFaith(current, target.posX, target.posY);
				#endregion
			}
		}
