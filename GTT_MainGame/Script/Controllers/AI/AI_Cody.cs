...
public class AI_Cody : MonoBehaviour {
	public static AI_Cody I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		//Uses the ability on a target enemy
		if(hasTarget){
			MG_ClassUnit target = getSingleAttackTarget (current, 5);
			MG_ControlCommand.I.gameAction_cody_blinkStrike(current, target.posX, target.posY);
		}
