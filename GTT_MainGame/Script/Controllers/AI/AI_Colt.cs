...
public class AI_Colt : MonoBehaviour {
	public static AI_Colt I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 50){
			if (enemyCount <= 2 && nearestEnemyRange <= 4) {
				#region "Quick Draw"
				MG_ClassUnit target = getSingleAttackTarget (current, 4);
				MG_ControlCommand.I.gameAction_colt_quickDraw(current, target.posX, target.posY);
				#endregion
			}else{
				#region "Dynamite"
				MG_ClassUnit target = getSingleAttackTarget (current, 6);
				MG_ControlCommand.I.gameAction_colt_dynamite(current, target.posX, target.posY);
				#endregion
			}
		}
