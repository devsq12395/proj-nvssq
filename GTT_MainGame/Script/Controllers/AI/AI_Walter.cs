...
public class AI_Walter : MonoBehaviour {
	public static AI_Walter I;
	...
	public void _runAI(MG_ClassUnit current, Vector2 target, int moveToDo){
		...
		switch(moveToDo){
			...
			case 3:					_move(current, target); break;
		...
		if(hasTarget && current.MP >= 50){
			if (enemyCount >= 2) {
				if (abiNum == 1) {
					...
					MG_ControlCommand.I.gameAction_walter_burstFire (current, target.posX, target.posY);
					#endregion
				} else {
					#region "Demonic Slash"
					MG_ControlCommand.I.gameAction_walter_demonicSlash (current);
					#endregion
				}
			}
		}
