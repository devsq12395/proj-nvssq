...
public class AI_Amy : MonoBehaviour {
	public static AI_Amy I;
	...
	public void useAbility(MG_ClassUnit current){
		...
		assesSituation (current, 7);

		//Uses the ability on a target enemy
		if(hasTarget && current.MP >= 50){
			if (nearestEnemyRange >= 6) {
				#region "Arc Shot"
				MG_ControlCommand.I.gameAction_amy_arcShot (current);
				#endregion
			}else{
				#region "Multishot"
				MG_ClassUnit target = getSingleAttackTarget (current, 6);
				MG_ControlCommand.I.gameAction_amy_multishot (current, target.posX, target.posY);
				#endregion
			}
		}
