...
public class AI_Siegfried : MonoBehaviour {
	public static AI_Siegfried I;
	...
	public void _move(MG_ClassUnit current, Vector2 target){
		...
		if(current.name == "Siegfried"){
			...
			MG_ControlSounds.I._playSound(32, current.posX, current.posY, true);
		}
		#endregion
		#endregion
	...
	public void useAbility(MG_ClassUnit current){
		...
		if(hasTarget && current.MP >= 50){
			...
			MG_ControlCommand.I.gameAction_siegfried_heavyCharge(current, target.posX, target.posY);
			#endregion
		}
